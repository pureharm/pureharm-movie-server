...
object AuthedHttp4s {
  ...
  def userTokenAuthMiddleware[F[_]: Async](
      authAlgebra: UserAuthAlgebra[F]): AuthMiddleware[F, AuthCtx] =
  ...
  private def onFailure[F[_]: Async]: AuthedService[Throwable, F] = Kleisli {
    _: AuthedRequest[F, Throwable] =>
      val fdsl = Http4sDsl[F]
      import fdsl._
      OptionT.liftF(Unauthorized(wwwHeader))
  }

  private def verifyToken[F[_]: Async](authAlgebra: UserAuthAlgebra[F])
    : Kleisli[F, Request[F], Attempt[AuthCtx]] =
    Kleisli { req: Request[F] =>
      ...
      optHeader match {
        case None =>
          Attempt
            .raiseError[AuthCtx](
              UnauthorizedFailure(s"No ${`X-Auth-Token`} provided"))
            .pure[F]
        case Some(header) =>
          authAlgebra
