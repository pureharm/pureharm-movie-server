...
final private[rest] class MovieRestRoutes[F[_]](
    ...
    with MovieRoutesJSON {

  implicit private val releaseDateQueryParamDecoder
    : QueryParamDecoder[ReleaseDate] =
    QueryParamDecoder[LocalDate].map(ReleaseDate.apply)

  private object StartReleaseDateQueryMatcher
      extends QueryParamDecoderMatcher[ReleaseDate]("start")
  private object EndReleaseDateQueryMatcher
      extends QueryParamDecoderMatcher[ReleaseDate]("end")
  ...
  private object TitleQueryParamMatcher
      extends QueryParamDecoderMatcher[TitleQuery]("title")

  private val imdbImportRoutes: AuthCtxRoutes[F] = {
    AuthCtxRoutes[F] {
      case PUT -> Root / "movie_import" / "imdb" :? TitleQueryParamMatcher(
            title) as user =>
  ...
  private val movieRoutes: AuthCtxRoutes[F] = {
    AuthCtxRoutes[F] {
      ...
      case GET -> Root / "movie" :? StartReleaseDateQueryMatcher(start) :? EndReleaseDateQueryMatcher(
            end) as user =>
