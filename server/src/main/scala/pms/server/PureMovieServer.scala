...
final class PureMovieServer[F[_]: Concurrent] private () {
  ...
  def init: F[(PureMovieServerConfig, ModulePureMovieServer[F])] = {
    for {
      ...
      pmsModule    <- moduleInit(gmailConfig, imdbAlgebraConfig, transactor)
  ...
  private def moduleInit(
    ...
    Concurrent.apply[F].delay(ModulePureMovieServer.concurrent(gmailConfig, imdblgebraConfig)(implicitly, transactor))
